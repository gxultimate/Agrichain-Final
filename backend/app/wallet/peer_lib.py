  ...
  def connectToInitialPeers(self, ip, port,branch):
    for peer in self.listOfInitialPeers:
      ...
      if ip != peer['ip'] or port != peer['port']:
        ...
        # sleep(2)
        try:
          server = socks_c(peer['ip'],peer['port'],LoggingNamespace, wait_for_connection=False)
          ...
          # sleep(1)
          print(f"{getTimeStamp()} >> Can't connect to {peer['branch']}")
...
#reciprocate connection with other nodes
  def reciprocateConnection(self, ip, port,branch):
    ...
    try:
      ...
      server = socks_c(ip, port, LoggingNamespace, wait_for_connection=False)
