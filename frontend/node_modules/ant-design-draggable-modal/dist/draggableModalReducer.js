...
export var draggableModalReducer = function (state, action) {
    ...
    switch (action.type) {
        case 'resize':
            ...
            return __assign({}, state, { maxZIndex: getNextZIndex(state, action.id), modals: __assign({}, state.modals, (_a = {}, _a[action.id] = __assign({}, state.modals[action.id], size, { zIndex: getNextZIndex(state, action.id) }), _a)) });
        case 'drag':
            return __assign({}, state, { maxZIndex: getNextZIndex(state, action.id), modals: __assign({}, state.modals, (_b = {}, _b[action.id] = __assign({}, state.modals[action.id], clampDrag(state.windowSize.width, state.windowSize.height, action.x, action.y, state.modals[action.id].width, state.modals[action.id].height), { zIndex: getNextZIndex(state, action.id) }), _b)) });
        case 'show': {
            ...
            return __assign({}, state, { maxZIndex: state.maxZIndex + 1, modals: __assign({}, state.modals, (_c = {}, _c[action.id] = __assign({}, modalState_1, position, size_1, { zIndex: state.maxZIndex + 1, visible: true }), _c)) });
        }
        case 'focus':
            var modalState = state.modals[action.id];
            return __assign({}, state, { maxZIndex: state.maxZIndex + 1, modals: __assign({}, state.modals, (_d = {}, _d[action.id] = __assign({}, modalState, { zIndex: state.maxZIndex + 1 }), _d)) });
        case 'hide': {
            var modalState_2 = state.modals[action.id];
            return __assign({}, state, { modals: __assign({}, state.modals, (_e = {}, _e[action.id] = __assign({}, modalState_2, { visible: false }), _e)) });
        }
        case 'mount':
            return __assign({}, state, { maxZIndex: state.maxZIndex + 1, modals: __assign({}, state.modals, (_f = {}, _f[action.id] = __assign({}, initialModalState, { x: state.windowSize.width / 2 - initialModalState.width / 2, y: state.windowSize.height / 2 - initialModalState.height / 2, zIndex: state.maxZIndex + 1 }), _f)) });
        case 'unmount':
            var modalsClone = __assign({}, state.modals);
            delete modalsClone[action.id];
            return __assign({}, state, { modals: modalsClone });
        case 'windowResize':
            return __assign({}, state, { windowSize: action.size, modals: mapObject(state.modals, function (modalState) {
                    ...
                    return __assign({}, modalState, position, size);
                }) });
        default:
            throw new Error();
    }
